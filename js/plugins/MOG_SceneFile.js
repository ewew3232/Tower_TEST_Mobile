 ...
 *
 * ※Plicy対策版
 * https://plicy.net/ ではMV製のゲームのセーブデータが全て同じ場所に入ります。
 * このプラグインが配布されたままの状態だと、エラーが起きるようです。
 * これについて、対策処理をしたものになります。
 * 情報提供元：
 *   https://twitter.com/kanjinokusa/status/1239098836166959104
 *
 * ===========================================================================
 * +++ MOG - Scene File (v1.0) +++
 * By Moghunter
 ...
 * ※ファイル名は小文字です。
 * サンプルプロジェクトはイニシャルが大文字ですが、小文字を読み込みます。
 *
 * /img/menus/file/
 *
 * background.png
 ...
 * 修正（追記）
 * ===========================================================================
 * v1.0f プラグインパラメーター Cursor File Space が動作していないバグを修正。
 *
 * ===========================================================================
 *
 */
...
//	Moghunter.scFile_CursorS = Number(Moghunter.parameters['File Space'] || 159);
	Moghunter.scFile_CursorS = Number(Moghunter.parameters['Cursor File Space'] || 159);
...
Scene_File.prototype.updateCommand = function() {
	...
	//if (TouchInput.isTriggered()) {this.checkTouchFile()}; //タッチ機能無効
};
...
Window_FileData.prototype.drawData = function() {
	  ...
	  this.resetTextColor();
	  this.drawText(this.data().playtime, w1 - 30, 190, 120, 'right');
